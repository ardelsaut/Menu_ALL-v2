# Scripts Nono
# 11.ssh and Remote Desktop
# V:\03.PC\01.WINDOWS\04.COMPILATION_SCRIPTS\Menu_ALL-v2\archives\Scripts\11.ssh_and_Remote Desktop.ps1
...
if ($computerName -match "portable") {
    ...
    if ($PortRule -eq "3389") {
        ...
        if (($ChangeRule -eq "Y") -or ($ChangeRule -eq "1")){
            ...
            Write-Output "le port de remote desktop est actuellement: $PortRule et restera donc tel quel."
        } else {
            Write-Output "Mauvaise Entrée"
            exit
        }
    }
